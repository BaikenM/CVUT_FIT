...
/* INFO: need either suffix array, DAWG or KMP for speed optimization
 *
 * A program that searches for word occurences in an input sequence using templates. 
 * The input sequence can be either a string, a template vector or a template list.
 * */
...
class CIndex
{
  public:
    ...
    : m_Sequence(sequence),
      ...
      m_SeqSize(m_Sequence.size()){}
    
    set<size_t> search(const T_& substr) const
    {
      set<size_t> res_seq;

      int i = 0, j = 0;
      for(i = 0; next(m_Sequence.begin(), i) != m_Sequence.end(); i++)
      {
        for(j = 0; next(m_Sequence.begin(), i + j)!= m_Sequence.end() 
        && next(substr.begin(), j) != substr.end() 
        && m_Comparator(*(next(m_Sequence.begin(), i + j)), *(next(substr.begin(), j))) == 0
        && m_Comparator(*(next(substr.begin(), j)), *(next(m_Sequence.begin(), i + j))) == 0
        ; j++);
        if(next(substr.begin(), j) == substr.end()) res_seq.insert(i);
      }
      return res_seq;
    }
...
// a string specialization to ease the parsing of strings
template<>
class CIndex<string>
{
  public:
    // constructor
    CIndex(const string& sequence)
    : m_Sequence(sequence),
      m_SeqSize(m_Sequence.size()){}

    set<size_t> search(const string& substr) const
    {
      set<size_t> res_seq;
      size_t      i = 0; 
      int         j = 0;
      
      for(i = 0; i < m_SeqSize; i++)
      {
        for(j = 0; m_Sequence[i + j] != '\0' 
        && substr[j] != '\0' 
        && m_Sequence[i + j] == substr[j]; j++);
        if(substr[j] == '\0') res_seq.insert(i);
      }
      return res_seq;
    }
  private:
